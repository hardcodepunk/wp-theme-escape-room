...
function escape_room_setup() {
	/*
		...
		* If you're building a theme based on Escape Room, use a find and replace
		* to change 'escape-room' to the name of your theme in all the template files.
		*/
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'escape-room' ),
		)
	);
...
function escape_room_scripts() {
	wp_enqueue_style( 'escape-room-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'escape-room-style', 'rtl', 'replace' );

	wp_enqueue_script( 'escape-room-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
